...
def generate_holiday_feature(start_time):
    ...
    '''
    angle = (float(360)/7)*initial_start_time.weekday()
    holiday_feature_output['weekday_x'] = round(math.cos(math.radians(angle)), 4)
    holiday_feature_output['weekday_y'] = round(math.sin(math.radians(angle)), 4)
    '''
...
def generate_symmetry_time_feature(data_dict, start_time_obj, tollgate, direction):
    symmetry_feature = {}
    hour_step = int((start_time_obj.hour - 12.5)*2)
    data_list = []
    if hour_step > 0:
        start_time_obj -= datetime.timedelta(hours=hour_step)
        for step in xrange(2):
            time_flag = start_time_obj - datetime.timedelta(minutes=step*20)
            start_str = datetime.datetime.strftime(time_flag, '%Y-%m-%d %H:%M:%S')
            if data_dict[start_str][tollgate][direction]['volume']:
                data_list.append(data_dict[start_str][tollgate][direction]['volume'])
    else:
        start_time_obj -= (datetime.timedelta(days=1) - datetime.timedelta(hours=abs(hour_step)))
        for step in xrange(2):
            time_flag = start_time_obj + datetime.timedelta(minutes=step*20)
            start_str = datetime.datetime.strftime(time_flag, '%Y-%m-%d %H:%M:%S')
            if data_dict[start_str][tollgate][direction]['volume']:
                data_list.append(data_dict[start_str][tollgate][direction]['volume'])
    if len(data_list) < 1:
        symmetry_feature['sym_avg'] = -1
        symmetry_feature['sym_max'] = -1
        symmetry_feature['sym_min'] = -1
    else:
        symmetry_feature['sym_avg'] = np.average(data_list)
        symmetry_feature['sym_max'] = np.max(data_list)
        symmetry_feature['sym_min'] = np.min(data_list)
    return symmetry_feature


def get_others_data(data_file):
    retrieved_data = NestedDict()
    raw_data = open(data_file).readlines()
    for i in xrange(1, len(raw_data)):
        line_data = raw_data[i].split(',')
        retrieved_data[line_data[1]][line_data[0]][line_data[2]] = float(line_data[3])
    return retrieved_data


def generate_train_feature_file(dict_input_data, raw_input_data, output_feature_file='train_feature_file'):
    ...
    for line_count in xrange(1, len(raw_input_data)):
        ...
        if datetime.date(2016, 9, 30) < start_time_obj.date() < datetime.date(2016, 10, 10) or \
                        start_time_obj.date() > datetime.date(2016, 10, 24)or \
                        start_time_obj.hour not in [6, 7, 8, 9, 15, 16, 17, 18]:
            continue
        tollgate_id = each_data[0]
        direction_id = each_data[2]
        tmp_out_str = '{0}\t{1}\t{2}'.format(
            tollgate_id, direction_id, dict_input_data[start][tollgate_id][direction_id]['volume'])
        if write_idx == 1:
            tmp_out_idx = ''
        tmp_index = 0
        ...
        '''
        # Feature : Symmetry time feature
        sym_feature = generate_symmetry_time_feature(dict_input_data, start_time_obj, tollgate_id, direction_id)
        for s_feature in sorted(sym_feature.keys()):
            tmp_index += 1
            tmp_out_str += '\t{0}:{1}'.format(tmp_index, sym_feature[s_feature])
            if write_idx == 1:
                tmp_out_idx += '{0}:{1}\n'.format(tmp_index, s_feature)
        # Feature : Ensemble other's predicted result
        tmp_index += 1
        tmp_out_str += '\t{0}:{1}'.format(tmp_index, other_data[start][tollgate_id][direction_id])
        if write_idx == 1:
            tmp_out_idx += '{0}: Other predicted result\n'.format(tmp_index)
        '''
...
def generate_test_feature_file(dict_input_data, test_file, output_feature_file='test_feature_file'):
    output_file = open('{0}-{1}'.format(output_feature_file, datetime.datetime.today().strftime('%Y-%m-%d')), mode='w')
    ...
    for line_count in xrange(0, len(test_input_data)):
        ...
        tmp_out_str = '{0}\t{1}\t{2}'.format(tollgate_id, direction_id, volume.rstrip())
        tmp_index = 0
...
if __name__ == '__main__':
    ...
    while i < len(input_opt):
        ...
        elif input_opt[i] == '-other':
            other_file = input_opt[i+1]
        elif input_opt[i] == '-h':
    ...
    #other_data = get_others_data(other_file)
    generate_train_feature_file(data_dict, raw_data)
    if test_file:
        generate_test_feature_file(data_dict, test_file)
