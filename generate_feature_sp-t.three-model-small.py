...
def generate_symmetry_time_feature(data_dict, start_time_obj, tollgate, direction, mode='train'):
    symmetry_feature = {}
    ...
    start_str = datetime.datetime.strftime(start_time_obj, '%Y-%m-%d %H:%M:%S')
    sym_time_feature = generate_timebased_feature(data_dict, start_str, tollgate, direction, mode)
    for n in sym_time_feature.keys():
        for i in sym_time_feature[n].keys():
            symmetry_feature['sym_{0}-{1}'.format(n, i)] = sym_time_feature[n][i]
...
def generate_train_feature_file(dict_input_data, raw_input_data, output_feature_file='train_feature_file'):
    ...
    for line_count in xrange(1, len(raw_input_data)):
        ...
        if datetime.date(2016, 10, 17) < start_time_obj.date() < datetime.date(2016, 10, 25) \
            ...
            cv_sym_feature = generate_symmetry_time_feature(dict_input_data, start_time_obj, tollgate_id, direction_id,
                                                            'cv_test')
            for s_feature in sorted(cv_sym_feature.keys()):
                tmp_cv_index += 1
                tmp_cv_out_str += '\t{0}:{1}'.format(tmp_cv_index, cv_sym_feature[s_feature])
