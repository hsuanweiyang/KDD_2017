...
def get_volume_data(data_file):
    ...
    for line_count in range(1, len(volume_raw_data)):
        ...
        if datetime.date(2016, 9, 30) < start_time_obj.date() < datetime.date(2016, 10, 8):
            continue
...
def get_route_capacity(link_data, tollgate_id):
    route_capacity = {}
    lines_capacity = 0
    for intersection in link_to_intersection[tollgate_id]:
        current_distance = 0
        for each_link in link_to_intersection[tollgate_id][intersection]:
            link_length = float(link_data[each_link]['length'])
            lines_capacity += link_length*float(link_data[each_link]['lanes'])/(current_distance+link_length/2)
            current_distance += link_length
    route_capacity['route_capacity'] = lines_capacity
...
def get_weather_data(data_file):
    ...
    for line_count in range(1, len(weather_data)):
...
def generate_timebased_feature(raw_feature_data, start_time, stop_time, tollgate, direction, mode='train'):
    ...
    def minute_feature():
        ...
        while minutefeature_start.date() > datetime.date(2016, 9, 18) and len(minute_total_volume) < 6:
            ...
            if mode == 'cv_test':
                if datetime.date(2016, 10, 10) < minutefeature_start.date() < datetime.date(2016, 10, 18) and \
    ...
    def day_feature():
        ...
        while dayfeature_start.date() > datetime.date(2016, 9, 18):
            ...
            if mode == 'cv_test':
                if datetime.date(2016, 10, 10) < dayfeature_start.date() < datetime.date(2016, 10, 18):
                    current_x += 1
...
def generate_holiday_feature(start_time):
    ...
    initial_start_time = datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')
    yesterday = initial_start_time.date() - datetime.timedelta(days=1)
    ...
    yesterday_state = day_state(yesterday)
    ...
    if today_simple_state == 0:
        ...
        if yesterday_state + tomorrow_state == 2 or yesterday_state + tomorrow_state == 0:
            today_complex_state = 1
        elif yesterday_state == 0:
    ...
    '''
    holiday_feature_output['yesterday'] = yesterday_state
...
def generate_train_feature_file(dict_input_data, raw_input_data, link_data, weather_data, travel_time_data,
    ...
    tollgate_connect_intersection = {'1': '2', '2': '1', '3': '3'}
    for line_count in range(1, len(raw_input_data)):
        ...
        if datetime.date(2016, 9, 30) < start_time_obj.date() < datetime.date(2016, 10, 8) or \
                        start_time_obj.date() > datetime.date(2016, 10, 17):
            continue
        tmp_out_str = '{0}\t1:{1}'.format(dict_input_data[start][stop][tollgate_id][direction_id]['volume'], direction_id)
        if line_count == 1:
        ...
        # Feature : tollgateID in binary
        for n in ['1', '2', '3']:
            tmp_index += 1
            if n == tollgate_id:
                tmp_out_str += '\t{0}:1'.format(tmp_index)
            else:
                tmp_out_str += '\t{0}:0'.format(tmp_index)
            if line_count == 1:
                tmp_out_idx += '{0}:tollgate({1})\n'.format(tmp_index, n)

        # Feature : Number of connected intersection of tollgate
        tmp_index += 1
        tmp_out_str += '\t{0}:{1}'.format(tmp_index, tollgate_connect_intersection[tollgate_id])
        if line_count == 1:
            tmp_out_idx += '{0}:#connected intersection of tollgate\n'.format(tmp_index)
        ...
        for link_feature in ['length', 'lanes', 'in_lanes']:
            ...
            if line_count == 1:
                tmp_out_idx += '{0}:link-{1}\n'.format(tmp_index, link_feature)

        # Feature : Route capacity
        route_capacity = get_route_capacity(link_data, tollgate_id)
        tmp_index += 1
        tmp_out_str += '\t{0}:{1}'.format(tmp_index, route_capacity['route_capacity'])
        if line_count == 1:
            tmp_out_idx += '{0}:route capacity\n'.format(tmp_index)
        ...
        velocity_data = generate_tollgate_velocity(travel_time_data, link_data, start_time_obj, tollgate_id)
        for velocity_feature in sorted(velocity_data.keys()):
            tmp_index += 1
            tmp_out_str += '\t{0}:{1}'.format(tmp_index, velocity_data[velocity_feature])
            if line_count == 1:
                tmp_out_idx += '{0}:{1}\n'.format(tmp_index, velocity_feature)

        start_time_obj = datetime.datetime.strptime(start, '%Y-%m-%d %H:%M:%S')
        if datetime.date(2016, 10, 10) < start_time_obj.date() < datetime.date(2016, 10, 18) \
            ...
            cv_history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id, direction_id,
                                                            'cv_test')
            cv_near_gate_history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id,
                                                                     near_tollgate[tollgate_id], 'cv_test')
            for n in sorted(cv_history_feature.keys()):
                ...
                for n in sorted(cv_near_gate_history_feature.keys()):
                    for i in sorted(cv_near_gate_history_feature[n].keys()):
                        tmp_cv_index += 1
                        tmp_cv_out_str += '\t{0}:{1}'.format(tmp_cv_index, cv_near_gate_history_feature[n][i])
        ...
        near_gate_history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id,
                                                               near_tollgate[tollgate_id])
        for n in sorted(history_feature.keys()):
            for i in sorted(history_feature[n].keys()):
                ...
                if line_count == 1:
                    tmp_out_idx += '{0}:{1}\n'.format(tmp_index, i)
        for n in sorted(near_gate_history_feature.keys()):
            for i in sorted(near_gate_history_feature[n].keys()):
                tmp_index += 1
                tmp_out_str += '\t{0}:{1}'.format(tmp_index, near_gate_history_feature[n][i])
                if line_count == 1:
                    tmp_out_idx += '{0}:(neargate){1}\n'.format(tmp_index, i)
...
def generate_test_feature_file(dict_input_data, test_file, link_data, weather_data, travel_time_data,
    ...
    for line_count in range(1, len(test_input_data)):
        ...
        tmp_out_str = '{0}\t1:{1}'.format(volume, direction_id)
        tmp_index = 1
