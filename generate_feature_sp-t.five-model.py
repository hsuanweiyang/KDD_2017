...
def generate_train_feature_file(dict_input_data, raw_input_data, link_file, weather_file, travel_time_data,
    ...
    for line_count in xrange(1, len(raw_input_data)):
        ...
        tmp_out_str = '{0}\t{1}\t{2}'.format(
            tollgate_id, direction_id, dict_input_data[start][stop][tollgate_id][direction_id]['volume'])
        if line_count == 1:
            tmp_out_idx = ''
        tmp_index = 0
        ...
        if datetime.date(2016, 10, 10) < start_time_obj.date() < datetime.date(2016, 10, 18) \
            ...
            cv_near_gate_history_feature = generate_timebased_feature(dict_input_data, start, stop,
                                                                      near_tollgate[tollgate_id], direction_id,
                                                                      'cv_test')
        ...
        near_gate_history_feature = generate_timebased_feature(dict_input_data, start, stop,
                                                               near_tollgate[tollgate_id], direction_id)
...
def generate_test_feature_file(dict_input_data, test_file, link_file, weather_file, travel_time_data,
