...
def generate_weather_feature(weather_file, start_time):
...
def generate_train_feature_file(dict_input_data, raw_input_data, link_file, weather_file,
    ...
    for line_count in xrange(1, len(raw_input_data)):
        ...
        tmp_out_str = '{0}\t{1}\t1:{2}'.format(
            tollgate_id, dict_input_data[start][stop][tollgate_id][direction_id]['volume'], direction_id)
        if line_count == 1:
        ...
        # Feature : Weather
        weather_data = generate_weather_feature(weather_file, start)
        for weather_feature in sorted(weather_data.keys()):
            tmp_index += 1
            tmp_out_str += '\t{0}:{1}'.format(tmp_index, weather_data[weather_feature])
            if line_count == 1:
                tmp_out_idx += '{0}:{1}\n'.format(tmp_index, weather_feature)
        ...
        if datetime.date(2016, 10, 10) < start_time_obj.date() < datetime.date(2016, 10, 18) \
            ...
            cv_history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id, direction_id, 'cv_test')
            op_cv_history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id, op_direction_id, 'cv_test')
            ...
            '''
            for n in sorted(op_cv_history_feature.keys()):
                for i in sorted(op_cv_history_feature[n].keys()):
                    tmp_cv_index += 1
                    tmp_cv_out_str += '\t{0}:{1}'.format(tmp_cv_index, op_cv_history_feature[n][i])
            '''
...
def generate_test_feature_file(dict_input_data, test_file, link_file, weather_file,
                               output_feature_file='test_feature_file'):
    output_file = open('{0}-{1}'.format(output_feature_file, datetime.datetime.today().strftime('%Y-%m-%d')), mode='w')
    test_input_data = open(test_file, mode='r').readlines()
    tollgate_link = {'1': '113', '2': '117', '3': '122'}
    tollgate_connect_intersection = {'1': '2', '2': '1', '3': '3'}
    for line_count in xrange(1, len(test_input_data)):
        ...
        if direction_id == '1':
            ...
            op_direction_id = '1'
        tmp_out_str = '{0}\t{1}\t1:{2}'.format(tollgate_id, volume, direction_id)
        tmp_index = 1
        ...
        # Feature : Volume data of previous time window (min, day, week)
        history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id, direction_id)
        op_direction_history_feature = generate_timebased_feature(dict_input_data, start, stop, tollgate_id,
                                                                  op_direction_id)
        for n in sorted(history_feature.keys()):
            for i in sorted(history_feature[n].keys()):
                tmp_index += 1
...
if __name__ == '__main__':
    ...
    generate_train_feature_file(data_dict, raw_data, link_file, weather_file)
    if test_file:
        generate_test_feature_file(data_dict, test_file, link_file, weather_file)
