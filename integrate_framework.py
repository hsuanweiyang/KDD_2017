...
class IntegratedPredictor:
    ...
    def svm(self):
        ...
        def generate_tmp_file(label, feature):
            tmp_train_file = open('tmp_train_file', mode='w')
            tmp_test_file = open('tmp_test_file', mode='w')
            feature_amount = len(feature[0])
            tmp_output = ''
            for i in range(len(label)):
                tmp_feature = ''
                for n in range(feature_amount):
                    tmp_feature = '{0}\t{1}:{2}'.format(tmp_feature, n + 1, feature[i][n])
                tmp_output = '{0}{1}{2}\n'.format(tmp_output, label[i], tmp_feature)
            tmp_train_file.write(tmp_output[0::2])
            tmp_test_file.write(tmp_output[1::2])
            tmp_test_file.close()
            tmp_train_file.close()
            return feature_dimension_amount
